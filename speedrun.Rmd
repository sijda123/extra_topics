...
> Please import, clean, tidy, and merge these files.
...
litters <- litters %>%
  mutate(
    ...
    gd18 = as.Date(gd18),
    group = factor(group),
    # there is no dose column
    dose = factor(dose, ordered = TRUE)
    # note: very minimal instructions given
  )
...
> This file contains data on three weather stations, and information like date, tmax, tmin, and precipitation. Please do exploratory analyses and visualizations, provide the R code to reproduce these.
...
> seperately, I have a dataset with airbnb rental information in NYC. this contains lat, long, price, and rating, among others. please create a leaflet map for these data, using greyscale background, circles to mark locations, and color according to price.
...
# tells you price, rating, lat, and long
```
...
> I have a new dataset coming from Australian researchers describing the relationship between posture and "enlarged protuberances" especially among younger subjects. Note the first 6 rows contain codebook information, not data. Please import, clean, and organize these data. Also note any issues in the resulting data file.
...
mtp <- mtp %>%
  mutate(
    ...
    sex = case_when(
      # used likely coding values for variables and not actual code book values
...
# this code was different if you ask the same prompt again
```
...
> Lastly, write a simulation for the birthday problem

Function

```{r}
# wrote a function of size n

birthday_sim <- function(n) {
  # Simulate n birthdays (1–365)
...
birthday_results <- 
  ...
  mutate(
    # this line of code was wrong, had to fix it
    match = map_lgl(group_size, \(n) birthday_sim(n))
  ) %>%
  group_by(group_size) %>%
  # take average across groups
...
# 50% probability is around group size of 23
# way faster than sitting down and writing it
```
